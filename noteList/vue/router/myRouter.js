...
手动实现一个vuerouter
 ...
 既然是一个插件那么我们就要为其实现一个install方法
 为什么不直接引入vue呢
 因为引入vue后webpack会将其一同打包，vue被反复打包
 VueRouter可以实现为一个构造函数也可以使用一个类去实现
 这个类接收参数option(常见的option值routes路由配置数组)
*/  
// let Vue;
// class VueRouter {
//     constructor(options){
...
//             render:(h)=>{
//                 let comp = this.routeMap[this.current];
//                 h(comp);
//             }
//         })
//     }
...
//         /*
//          利用mixin对vue实例进行扩展
//          mixin混入的方法会对vue的每一个实例都生效
//          所以我们需要进行控制，让她只对根组件生效一次

//         */  
//         beforeCreate(){
//             if(this.$options.router){
//                 // 将在根组件中的router对象挂载到vue的原型上
